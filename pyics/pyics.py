...
class pyicsTests(unittest.TestCase):
	...
	def testi8c(self):
		...
		self.assert_(N.sum(self.img_i8c_rec['data'] - self.img_i8) == 0,
		 	msg="Storing 8 bits integer data with compression failed (diff: %d)" % (N.sum(self.img_i8c_rec['data'] - self.img_i8)))
	...
	def testi8u(self):
		...
		self.assert_(N.sum(self.img_i8u_rec['data'] - self.img_i8) == 0,
			msg="Storing 8 bits integer data without compression failed (diff: %d)" % (N.sum(self.img_i8u_rec['data'] - self.img_i8)))
	...
	def testi16c(self):
		...
		self.assert_(N.allclose(self.img_i16c_rec['data'], self.img_i16),
			msg="Storing 16 bits integer data with compression failed (diff: %d)" % (N.sum(self.img_i16c_rec['data'] - self.img_i16)))

	def testi16u(self):
		"""Test int 16 ucompressed functions"""
		write('/tmp/pyics-testi16u', self.img_i16, 0, 'testcase', 1)
		self.img_i16u_rec = read('/tmp/pyics-testi16u', 1)
		self.assert_(N.allclose(self.img_i16u_rec['data'], self.img_i16),
			msg="Storing 16 bits integer data without compression failed (diff: %d)" % (N.sum(self.img_i16u_rec['data'] - self.img_i16)))
	...
	def testf32u(self):
		...
		self.assert_(N.allclose(self.img_f32_rec['data'], self.img_f32),
			msg="Storing 32 bits float data without compression failed (diff: %g)" % (1.0*N.sum(self.img_f32_rec['data'] - self.img_f32)))

	def testf32c(self):
		"""Test if float 32 compressed fails"""
		with self.assertRaises(RuntimeError()):
			write('/tmp/pyics-testf32', self.img_f32, 1, 'testcase', 1)
